...
class Clothes(db.Model):
    ...
    def __init__(
        self, clothesID, size, name, description, color, cost, image, isOnPromotion
    ):
    ...
    def json(self):
        return {
            ...
            "cost": self.cost,
            "image": self.image,
            "isOnPromotion": self.isOnPromotion,
        }
...
def get_all_clothes():
    ...
    if len(clotheslist):
        return jsonify(
            {
                "code": 200,
                "data": {"clothes": [clothe.json() for clothe in clotheslist]},
            }
        )
    return jsonify({"code": 404, "message": "There are no such clothes."}), 404
...
def find_by_clothesID(clothesID):
    ...
    if clothes:
        return jsonify({"code": 200, "data": clothes.json()})
    return jsonify({"code": 404, "message": "clothes not found."}), 404
...
def get_all_customer():
    ...
    if len(custlist):
        return jsonify(
            {
                "code": 200,
                "data": {"Customer": [customer.json() for customer in custlist]},
            }
        )
    return jsonify({"code": 404, "message": "There are no such customers."}), 404
        ...
        return jsonify(
            {
                "code": 200,
                "data": {
                    "CustomerEmail": customerMe.customerEmail,
                    "PhoneNum": customerMe.phoneNum,
                },
            }
        )
    return jsonify({"code": 404, "message": "There are no such customers."}), 404
...
def get_all_numbers():
    ...
    if len(custlist):
        return jsonify(
            {"code": 200, "data": {"num": [num.phoneNum for num in custlist]}}
        )
    return jsonify({"code": 404, "message": "There are no such numbers."}), 404
...
class Order(db.Model):
    ...
    customerEmail = db.Column(
        db.String(32), db.ForeignKey(customer.customerEmail), nullable=False
    )
    clothesID = db.Column(
        db.Integer(), db.ForeignKey(Clothes.clothesID), nullable=False
    )
...
def get_all_order():
    ...
    if len(orderlist):
        return jsonify(
            {"code": 200, "data": {"order": [order.json() for order in orderlist]}}
        )
...
def create_order():
    ...
    try:
        ...
        return (
            jsonify({"code": 500, "message": "An error occurred creating the order."}),
            500,
        )
...
def update_order(order_id):
    if Order.query.filter_by(order_id=order_id).first():
        ...
        return jsonify({"code": 201, "message": "Total Price has been updated"}), 201
...
class Invoice(db.Model):
    ...
    def __init__(
        self, customerEmail, order_ID, description, totalPrice, payMethod, stripe_id
    ):
        self.customerEmail = customerEmail
        self.order_ID = order_ID
        self.description = description
...
def get_all_invoice():
    ...
    if len(invoicelist):
        return jsonify(
            {
                "code": 200,
                "data": {"invoice": [invoice.json() for invoice in invoicelist]},
            }
        )
    return jsonify({"code": 404, "message": "There are no such invoices."}), 404
...
def find_invoice_by_orderID(order_ID):
    ...
    return jsonify({"code": 404, "message": "invoice not found."}), 404
...
def create_invoice():
    ...
    try:
        ...
        return (
            jsonify(
                {"code": 500, "message": "An error occurred creating the invoice."}
            ),
            500,
        )
...
def update_invoice(order_id, data):
    if Invoice.query.filter_by(order_ID=order_id).first():
        ...
        return jsonify({"code": 201, "message": "Stripe ID has been updated."}), 201
...
def make_payment():
    ...
    if request.is_json:
        try:
            ...
            print('EXCEPTION IN MAKEPAYMENT')
...
def finalizePayment(invoice_id, session_id=None):
    ...
    print("order_result:", order_result.json)

    totalMe = 0
    clothesMe = []
    for i in order_result.json["data"]["order"]:
        # print(i)
    ...
    invoice = {
        "customerEmail": "ahkao@gmail.com",
        "order_ID": invoice_id,
        "description": finalClothes,
        "totalPrice": totalMe,
        "payMethod": "Card",
        "stripe_id": "_blank",
    }
    invoice_result = create_an_invoice(invoice)
    print("invoice_result:", invoice_result[0].json)
    print("\nOrder sent to invoice.\n")

    code = invoice_result[0].json["code"]
    if code not in range(200, 300):
        return {
            "code": 500,
    ...
    stripe_id = update_invoice(invoice_id, payment_intent)
    print("stripe_id_result:", stripe_id)
    code = stripe_id[0].json["code"]
    if code not in range(200, 300):
        return {"code": 500, "message": "Update of Invoice Stripe ID failure."}
...
def create_checkout_session(product_ids):
    ...
    print(checkout_session.id, 'checkout_session_success_url')
    finalizePayment(productList[-1],checkout_session.id)
    # checkout_session_success_url = checkout_session.success_url.replace(
...
def delete_order(order_id):
    if Order.query.filter_by(order_id=order_id).first():
        ...
        try:
            ...
            return (
                jsonify(
                    {"code": 500, "message": "An error occurred deleting the order."}
                ),
                500,
            )
